...
$$
\alpha^i_ {u^t, v^t} = \frac{exp(LeakyReLU(a^T_ {r,i}[h_ {u_ t} \vert h_{v_ t}]))}{\sum_  {k^t \in \nu_ r(v^t)}exp(LeakyReLU(a^T_ {r,i}[h_ {k^t} \vert h_ {v_ t}]))}
$$
...
$$
TgaHead_i = \sum_{v^t \in \tilde{V}} \sigma(\sum_{u^t \in \nu_r(v^t)}\alpha^i_{u^t, v^t}h_{u^t})\\

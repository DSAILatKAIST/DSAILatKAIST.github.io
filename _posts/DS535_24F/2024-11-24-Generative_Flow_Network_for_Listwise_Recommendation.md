---
title: "[KDD 2024] Generative Flow Network for Listwise Recommendation"
permalink: 2024-11-24-Generative_Flow_Network_for_Listwise_Recommendation.html
tags: [reviews]
use_ math: true
usemathjax: true
---

# [KDD 2024] Generative Flow Network for Listwise Recommendation
...
\text{where } \tau=\{\emptyset=O^{0}\rightarrow O^{1}\rightarrow\cdots\rightarrow O^{T}\}
$$
...
$$
\mathcal{L}_{\text{TB}}(\theta, \phi)=\log\left(\frac{\mathcal{F}_{\phi}(u, \emptyset)\prod_{t=1}^{K}P_{\theta}(a_t\vert u, O^{t-1})}{\mathcal{R}(u, O^{K})}\right)^2
$$

TB는 object를 생성하는 경로 전체를 가지고 학습을 진행하기 때문에, 경로의 길이가 길어지는 경우 학습의 variance가 커질 위험이 있다. DB는 경로 전체가 아닌 transition을 기반으로 학습을 진행한다. <span style="color:blue">여기서 transition이란 전체 경로 $\tau=\{\emptyset=O^{0}\rightarrow O^{1}\rightarrow\cdots\rightarrow O^{T}\}$의 intermediate step $(O^{t-1}\rightarrow O^{1})$을 말한다.</span> DB의 목적함수는 아래와 같다.

$$
\mathcal{L}_{\text{DB}}(\theta, \phi)=\sum_{t=1}^{K-1}\log\left(\frac{\mathcal{F}_{\phi}(u, O^{t-1})P_{\theta}(a_t\vert u, O^{t-1})}{\mathcal{F}_{\phi}(u, O^t)}\right)^2+\log\left(\frac{\mathcal{F}_{\phi}(u, O^{K-1})P_{\theta}(a_t\vert u, O^{K-1})}{\mathcal{R}_{\phi}(u, O^K)}\right)^2
$$

<span style="color:blue">DB 목적함수를 최소화하는 것과 TB 목적함수를 최소화는 것 모두 loss가 0으로 수렴한다면 우리가 원하는 $P_{\theta}(\tau)\propto \mathcal{R}(O^{T})$를 만족하는 정책 함수를 얻을 수 있음이 증명되었다.</span>
...
2. 매 timestep $t$마다 아직 선택 안 된 아이템 후보 리스트 중 어떤 아이템을 추천 리스트에 넣을 것인지를 item selection model $P_{\theta}(i\vert u, O^{t-1})$로부터 선택한다. $a_t\sim P_{\theta}(i\vert u, O^{t-1})$
3. 선택한 아이템을 추천 리스트에 넣는다. $O^{t}=O^{t-1}\oplus\{a_t\}$
...
$$
P_{\theta}(O\vert u)=\prod_{t=1}^{K}P_{\theta}(a_t\vert u, O^{t-1})
$$

![alt text](https://i.postimg.cc/VLwG5Mc3/figure1.png)
...
본 단락에서는 사용자와 관련된 정보를 처리하는 Encoder구조에 대해 이야기한다. 사용자와 관련된 정보로는 user profile $(\mathcal{X}_ {u})$와 user history $(\mathcal{H}_ {u}= \{(a_ 1, Y_ {a_ 1}), \cdots, (a_ L, Y_ {a_ {L}})\})$이 주어진다. 본 논문에서는 Transformer기반 Encoder를 활용하여 user정보를 input으로 받아 user state embedding $s_u$를 만들고 이를 item selection model의 input으로 활용한다.

전체 모델 아키텍쳐는 아래 그림과 같다. <span style="color:blue">User Request Encoder로부터 얻음 user state embedding $s_t$와 item history로부터 얻은 item embeddings $\{e_{a_1},\cdots,e_{a_{t-1}},0,\cdots 0\}$을 concat하여 얻은 최종 embedding을 서로 다른 MLP Head를 통과하여 각각 $F_{\phi}$, $P_{\theta}$를 얻는다.</span>
![alt text](https://i.postimg.cc/RCKpMd27/figure2.png)
...
![alt text](https://i.postimg.cc/1tNCVrXZ/figure3.png)
...
![alt text](https://i.postimg.cc/kGQTfMW1/figure4.png)
...
1. Greedy vs Exploration
    ...
    ![alt text](https://i.postimg.cc/sxdnYGGG/figure5.png)
...
2. TB vs DB
    ...
    ![alt text](https://i.postimg.cc/9fkgfnWp/figure6.png)
...
$$
\hat{\mathcal{F}}(u, \emptyset)^{(i)} = \log\mathcal{R}(u, O^K_{i}) - \sum_{t=1}^{K}\log P_{\theta}(a_t\vert u, O^{t-1}_{i})\\

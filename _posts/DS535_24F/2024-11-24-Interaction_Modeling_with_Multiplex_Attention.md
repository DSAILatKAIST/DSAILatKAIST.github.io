...
permalink: 2024-11-24-Interaction_Modeling_with_Multiplex_Attention.html
tags: [reviews]
use_math: true
usemathjax: true
---
...
이 때 hyperparameter K를 얼마로 설정하는지에 따라 얼마나 복잡하고 추상적인 상호작용까지 모델링을 할지가 결정되므로, 모델링하고자 하는 데이터에 대한 도메인 지식을 기반으로 K를 결정하면 됩니다. 다만, PLT의 특성상 뒤에 학습되는 graph layer는 앞서 학습된 graph layer에 영향을 미치지 않기 때문에, K의 개수가 결과에 큰 영향을 미치지는 않습니다. (즉, K=4일 때 1--3번째로 학습된 graph layer의 값은 K=3일 때 1--3번째로 학습된 graph layer 값과 완전히 동일합니다. 4번째 graph layer가 학습될 때에는 앞선 3개 layer를 산출하는 encoder weights가 고정되기 때문이죠.)

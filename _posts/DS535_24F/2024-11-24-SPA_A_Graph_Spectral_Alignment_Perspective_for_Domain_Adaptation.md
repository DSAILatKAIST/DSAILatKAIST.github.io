...
permalink: 2024-11-24-SPA_A_Graph_Spectral_Alignment_Perspective_for_Domain_Adaptation.html
tags: [reviews]
use_math: true
usemathjax: true
---
...
SPA contains an adversarial domain adaptation framework, which includes a **feature encoder** $F(\cdot)$, a **classifier** $C(\cdot)$, and a **domain classifier** $D(\cdot)$. The feature encoder extracts features from input data, the classifier predicts class labels based on these features, and the domain classifier distinguishes between source and target domain features.
...
In order to model relationships between different samples within a give domain, SPA constructs self-correlation graphs for both the source and target domains. The nodes in the graph represent the feature embeddings of the samples from the feature encoder. The edges are weighted similarity between the node features, based on a similarity function such as cosine similarity. The graphs are dynamic in nature as they evolve as the feature encoder updates during training.
...
-  **Dynamic Nature**: The graphs are dynamic because $f_ i^s$ and $f_ i^t$ evolve as the feature encoder $F$ updates during training.
...
Based on the constructed correlation graphs, the domain alignment could be phrased as a graph alignment problem. The graph spectral alignment is achieved by minimizing the spectral distance between the Laplacian eigenvalues of the source and target domain graphs. This implicit alignment avoids the need for explicit, restrictive point-wise matching.

SPA introduces a spectral regularizer to align the source and target graphs:

  

1.  Compute the **graph Laplacian** $L$ for each graph.

2.  Obtain the **eigenvalues** $\Lambda$ of the Laplacian matrices for both source ($\Lambda^s$) and target ($\Lambda^t$) graphs.

3.  Define the **Spectral Distance**: $L_ {\text{gsa}} = \left\vert  \Lambda^s - \Lambda^t \right\vert_ p$ ,where $\left\vert  \cdot  \right\vert_ p$ denotes the $L_ p$ norm.
...
After the spectral alignment, rich topological information is transfered to the target domain. In order to enhance discriminability within the target domain, SPA introduces a neighbor-aware self-training mechanism. This mechanism generates pseudo-labels for each target sample based on the top k nearest labelled samples using K-Nearest Neighbor (KNN) algorithm. 

  

-  **Pseudo-label Generation**: Let $p_ i = C(F(x_ i))$ denote prediction probabilities for target samples $x_ i^t$, and $p _i^m$ denote the mapped probability stored in the memory bank. 

	<!-- - The pseudo label generation is a weighted K-Nearest Neighbors (kNN) algorithm to assign pseudo-labels to target samples. -->

  

-  **Voting Mechanism**: Each target sample $x_ i^t$ considers its $k$ nearest neighbors $N_ i$. The neighbors of the node vote for the class labels, weighted by their predicted probabilities.

  
	
	- Compute the weighted vote for class $c$ as $q_ {i,c} = \sum_ {j \in N_ i} p_ {j,c}^m$ and normalize to get the confidence $\hat{q}_ {i,c}$, which is computed as $\hat{q}_ {i,c} = \frac{q_ {i,c}}{\sum_ {m=1}^{C_ t} q_ {i,m}}$
...
-  **Memory Bank**: The Memory Bank plays a vital role in the neighbor-aware propagation mechanism by storing sharpened predictions and normalized features for target samples. Sharpening is achieved through the equation:
...
### 5.2 Results
 ...
 In the  **DomainNet** dataset, Table 1, SPA achieved an 8.6% improvement in accuracy over previous state-of-the-art methods, demonstrating superior performance in handling large-scale datasets with significant domain gaps. 
 
<img src="https://i.postimg.cc/fT8t142r/Table1.jpg" alt="Image description" width="70%">



 On the **OfficeHome** dataset, SPA showed approximately a 2.6% improvement in accuracy over existing methods, consistently outperforming competitors in all 12 adaptation scenarios. The results are presented in Table 2.
 
 <img src="https://i.postimg.cc/15mVWRHF/Table-2.jpg" alt="Image description" width="70%">
 
 For the **Office31 and VisDA2017** datasets, SPA performed on par with or better than state-of-the-art methods, showcasing its robustness and effectiveness across all domain pairs and tasks. In terms of evaluation metrics, classification accuracy on target domains was used, and SPA consistently showed improvements when compared to methods like DANN, CDAN, and MDD across all datasets. Refer to Table 3 for detailed results.

<img src="https://i.postimg.cc/6pT2XHF5/Table-3.jpg" alt="Image description" width="70%">

The authors also conduct t-SNE visualization to show the learning ability of SPA. They present the embeddings of source and target domain performed by SPA and three Baseline models DANN[1], BSP[3], and NPL[4] in figure 2. The authors claim that (i) embedding clusters by SPA are more compact, signifying the better discriminability achieved, and (ii) the source and target domain embeddings are more aligned in SPA, the red markers are closer to the blue markers, indicating better transferability. **However, the authors do not present any quantitative evidence to support these claims, leaving their conclusions open to question.**

<img src="https://i.postimg.cc/8zWtVQsN/Figure2.jpg" alt="Image description" width="70%">
...
-  **Paper Reference**:
   ...
   - [2] Baochen Sun and Kate Saenko. Deep coral: Correlation alignment for deep domain adaptation. In ECCV 2016 Workshops, 2016.
	...
	- [4] Lee, Dong-Hyun. "Pseudo-label: The simple and efficient semi-supervised learning method for deep neural networks." Workshop on challenges in representation learning, ICML. Vol. 3. No. 2. 2013.

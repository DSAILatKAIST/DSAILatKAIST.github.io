...
permalink: 2024-11-24-On_Evaluation_Metrics_For_Graph_Generative_Models.html
tags: [reviews]
use_math: true
usemathjax: true
---
...
또, Naeem et al, 2020. 에 따르면 생성된 그래프의 Fidelity와 Diversity는 다른 생성 Task와 마찬가지로 Precision 과 Recall을 통해 각각 측정되곤 했는데, 이러한 방법들로는 실제 데이터와 생성된 데이터의 분포가 동일한 경우 감지되기 어렵다는 점, 이상치에 민감하여 결과의 신뢰성이 낮아진다는 점이 한계로 꼽힌다. 뿐만 아니라 특정 데이터 카테고리가 아예 생성되지 않는 mode collapse 문제를 포착하는 것에도 한계가 있다. 만약 mode가 하나씩 점진적으로 사라지는 경우에는 Recall과 Precision이 모두 감소하면서 diversity가 감소하며 다양성 저하가 잘 반영되지만, mode들이 동시에 사라지는 경우 recall은 1개의 mode만 남을 때 까지 감소되지 않다가 특정 시점에서 급격하게 감소하는 등의 문제가 실험을 통해 밝혀진 것이다. 
...
$
\hat{\rho}(\mathbb{S}_ g, \mathbb{S}_ r) \approx \rho(P_ g, P_ r)
$
...
그래프 통계에 기반한 지표는 GGM 평가에서 표준으로 사용된다 (You et al., 2018; Liao et al., 2019; Dai et al., 2020). 이러한 지표는 $\mathbf{x}_ i$를 클러스터링 계수, node 차수, 또는 4-node 오빗 카운트 히스토그램으로 설정하고, 생성된 집합 $\mathbb{S}_ g$와 참조 집합 $\mathbb{S}_ r$ 사이의 경험적 MMD를 계산한다 (Gretton et al., 2006):
...
$
k(\mathbf{x}_ i, \mathbf{x}_ j) = \exp(-d(\mathbf{x}_ i, \mathbf{x}_ j) / 2\sigma^2)
$
...
또한, "유효한 그래프의 비율"과 같은 도메인 특화 지표도 존재한다. 우리의 목표는 이러한 지표를 통합하거나 제거하거나 평가하는 것이 아니다; 그것들은 생성된 그래프의 특성이며, 위에서 설명한 지표들과 달리 참조 분포와의 비교를 제공하지 않는다. 이러한 지표들은 GGM 평가에서 여전히 유용한 정보를 제공할 수 있다고 여겨진다.

GGM에서 MMD의 활용의 한계에 대한 또다른 연구인 O'Bray et al., 2022는 다음과 같은 한계점을 지적하기도 하였다.


### 2.1.a. 커널 및 하이퍼파라미터 설정에 대한 민감성

MMD는 어떤 kernel 함수와 hyperparameter(i.e. \sigma) 를 선택하느냐에 따라 크게 다른 결과를 보인다. 이는 평가 지표로 할용되기에 MMD가 가지고 있는 불안정성을 보여준다.

### 2.1.b. 스케일에 대한 부재

MMD 값은 적절한 척도를 가지고 있지 않아, 계산된 값이 의미하는 바를 해석하기 어렵다. 가령 0.1과 0.01이라는 값이 있을 때, 이 둘이 얼만큼의 차이를 의미한다는 것인지는 해당 Graph의 Context 없이 직관적으로 이해하기 어렵다. 이 역시 평가지표로써 MMD가 가지고 있는 한계로 여겨진다. 

### 2.1.c. 비단조성 

만약 두 graph가 dissimilar 한 정도가 증가한다면, 둘 사이의 MMD 역시 단조적으로 증가해야 마땅할 것이나 MMD는 비단조적으로 증가하는 특성을 보인다는 점에서, metric으로써 충분치 않다는 단점을 보인다. 이는 아래 figure에서 잘 드러난다.

![alt text](../../images/DS535_24F/On_Evaluation_Metrics_For_Graph_Generative_Models/image-5.png)
...
$
\mathbf{h}_v^{(l)} = \operatorname{MLP}^{(l)}( \mathbf{h}_ v^{(l-1)} + f^{(l)}( \{ \mathbf{h}_ u^{(l-1)} : u \in \mathcal{N}(v) \} ) )
$
...
$
\mathbf{x}_i = \operatorname{CONCAT}( \operatorname{READOUT}( \{ \mathbf{h}_v^{(l)} \mid v \in \mathbb{V} \} ) \mid l = 1, 2, \ldots, L )
$
...
첫 번째 실험에서는 $\mathbb{S}_g$의 품질에 영향을 주기 위해 랜덤 그래프를 활용했다. $\mathbb{S}_r$와 $\mathbb{S}_g$ 사이의 유사성을 감소시키기 위해, $\mathbb{S}_g(t)$에서 랜덤 그래프를 실제 그래프와 섞는 비율 $t$를 서서히 증가시켰다. 동시에 실제 그래프를 제거하여 $\vert \mathbb{S}_g \vert$가 전체적으로 일정하게 유지되도록 했다. 랜덤 그래프는 $\mathbb{S}_r$와 유사하도록 크기와 $p$ 값을 선택한 Erdős-Rényi(E-R) 그래프(Erdős & Rényi, 1960)이다.

...
permalink: 2024-11-24-Deep_Graph_Reprogramming.html
tags: [reviews]
use_math: true
usemathjax: true
---
...
- $X$\: discrete random variable with probabilities $P(X = k) \propto G_k$ 
- $G_k$: Gumbel random variables (i.e., noise) 
- $\tau$: softmax temperature

$\tau \to 0 \implies \text{argmax}, \quad \tau \to \infty \implies \text{uniform distribution}$
...
- $D$: downstream data distribution 
- $(x, y)$: downstream graph features와 그에 대응하는 레이블 
- $\vert \vert$: feature padding을 나타내는 결합 연산 
- $\delta_\text{padding}$: 최적화된 padding features 
- $GNN_\text{pre-trained}$: 사전 학습된 GNN 
- $L_\text{downstream}$: downstream loss 함수
...
- $\{u_i, v_i\}$: connection between node \(u\) and \(v\) 
- $m$: total number of edges in the input graph \(G\) 
- $L_\text{downstream}$: loss function for the downstream task
- $\alpha_{u_i, v_i}$: unary edge features 
- $\frac{\partial L_\text{downstream}}{\partial \alpha_{u_i, v_i}}$: edge gradient
...
- $\{u_i, v_i\}$: 그래프 \(G\)에서의 엣지 
- $m$: 그래프 \(G\)의 총 엣지 개수 
- $L_\text{downstream}$: 다운스트림 작업의 손실 함수 
- $\alpha_{u_i, v_i}$: unary 엣지 특징
- $G$: 수정된 그래프
...
![benchmarks](https://i.postimg.cc/Nj4PM9MH/Screenshot-2024-10-03-at-6-50-58-PM.png)
<p align="center">Table 1</p>

또한 모든 실험은 NVIDIA GeForce RTX 2080 Ti GPU 한대를 사용하여 수행하고 있습니다.

실험에서 사용되는 metric으로는 다음과 같습니다.
-  Accuracy (Acc)
	- 분류 문제에서 올바르게 분류된 샘플 비율로, 모델의 전반적인 성능을 평가.
	- Graph classification task 평가, Homogeneous domain에서 EdgSlim을 적용한 node classification 성능 평가, Pre-trained DGCNN (ModelNet40)을 ShapeNet downstream classes에 적용하여 cross-domain task 성능 평가.
-  Mean Absolute Error (MAE) 
	- 예측값과 실제값 간의 절대 차이 평균으로, 회귀 모델의 정확성을 나타냄.
	- Graph regression task 평가.
- ROC-AUC
	- 이진 분류 문제에서 ROC 곡선 아래의 면적으로, 분류 능력을 평가.
	- Molecular classification task.
- Root Mean Square Error (RMSE)
	- 예측값과 실제값 간의 오차 제곱 평균의 제곱근으로, 회귀 모델의 오차를 평가.
	- Molecular regression task.
- Semantic Similarity (Intermediate Feature Space)
	- 중간 특징 공간에서 구조적 유사도를 평가하여 데이터 표현의 일관성을 확인.
...
본 실험은 Citeseer dataset으로 사전 훈련을 하고, 다양한 heterogenous dimension을 가진 데이터셋에 대해 사전 훈련된 모델을 적용한 결과입니다.<br> MetaFP는 모델을 retraining을 하지 않고 진행하였기 때문에, retraining을 한 모델들보다 성능이 살짝 못미치지만 비교할 수 있을 만한 성능을 보여주고 있습니다. 특히 MERE의 ReAgg approach를 더해준 결과, MetaFP만을 사용한 결과보다 평균적으로 2.3% 향상된 모습을 보여주고 있습니다.<br> Cora 데이터셋 같은 경우 ReAgg 방법을 사용했을 시 결과가 조금 하락하는 것을 볼 수 있는데, 이는 Citseer와 Cora 데이터셋이 모두 computer science domain의 데이터 셋으로 domain 차이가 나지 않아 ReAgg 방법이 효과적이지 않는 것을 볼 수 있습니다. 

![exp2](https://i.postimg.cc/vmx9KkpZ/3.png)
<p align="center">Table 3</p>
Table 3 는 padding size와 padding하는 위치에 변화를 주고, 다양한 pre-trained task와 downstream task에 MetaFP를 적용하여 성능을 비교한 결과입니다.<br>여기서 주목할 만한 점은 작은 padding size와 random한 position에 padding을 진행하여도 높은 성능을 보여주고 있다는 것입니다. 
...
Table 4는 node-level classification을 진행한 하나의 pre-trained model로 cross-level task(graph regression, classifcation)를 진행한 결과입니다.<br> MetaGP를 이용하여 downstream task를 진행하였고, vanalia reusing 방법을 사용한 것보다 모두 압도적으로 우수한 성능을 이끌어낸다는 것을 볼 수 있습니다. 

![exp5](https://i.postimg.cc/fWfd1xgm/6.png)
<p align="center">Table 5</p>
Table 5는 homogenous domain에서 node classification을 진행하였습니다. <br>
EdgSlim은 retraining을 하지 않고도 vanila method는 물론이고, retraining을 한 모델들과 견주어도 비교할 만한 결과를 도출해 내는 것을 볼 수 있습니다. 

## 4.2. Homogenous - Domains

![exp6](https://i.postimg.cc/J4TkzR8h/7.png)
<p align="center">Table 6</p>
Homogenous domain에서의 첫번째 task는 molecular property를 예측하는 것으로, MetaGP를 적용하여 하나의 pre-trained 모델로 cross-level task 진행한 것을 볼 수 있습니다. <br>
이 실험 역시 vanilla method 보다 뛰어난 성능을 보여주고 있습니다.

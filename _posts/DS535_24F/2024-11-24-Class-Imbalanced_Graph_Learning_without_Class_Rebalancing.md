...
permalink: 2024-11-24-Class-Imbalanced_Graph_Learning_without_Class_Rebalancing.html
tags: [reviews]
use_math: true
usemathjax: true
---
...
![2024-11-17-115827.png](https://i.postimg.cc/5NWvdcHW/2024-11-17-115827.png)
...
![2024-11-17-115952.png](https://i.postimg.cc/0QJ6FZPN/2024-11-17-115952.png)
...
* Dataset
	...
	* 공동 저자 네트워크(Co-author networks): CS, Physics
	...
	![2024-11-17-121945.png](https://i.postimg.cc/8cpSMPKV/2024-11-17-121945.png)
...
* Evaluation Metric
	...
	* &emsp; $BAcc. = \frac{1}{m} \sum_ {i=1}^ {m}{Acc.(c_ {i})}$
	
	* &emsp; $\text{PerfStd}=\sqrt{\frac{1}{m}\sum_ {i=1}^ {m}{(Acc(c_ {i})-BAcc.)^ {2}}}$

#### **Adjusting Class Imbalance**
* **Step Imbalance ($\text{IR}=10, 20$)**

	* 전체 class 중에서 절반을 minority class로 나눈다. ($\lfloor m/2 \rfloor$)

	* 그리고 나머지는 majority class가 된다.

	* 이후에 minority class에서 무작위로 training 노드를 제거하는 방식으로 IR을 맞춘다.
* **Natural Imbalance ($\text{IR}=50, 100$)**
	
	* 실제 real-world 데이터에는 power-law 분포를 따르는 long-tail 형태의 분포가 흔하다.

	* Long-tail 분포를 갖도록 설정하기 위해서, 이때 가장 많은 클래스의 노드 수가 IR의 값과 일치한다. ($n_ {head}=\text{IR}$)

	* 가장 작은 클래스의 노드 수는 1이다.

	* k번째 클래스의 training 노드 개수는 $n_ {k}=\lfloor n_ {head}^ {\lambda_ {k}} \rfloor, \lambda_ {k}=\frac{m-k}{m-1}$ 
  
![2024-11-17-124057.png](https://i.postimg.cc/XqzzqVtL/2024-11-17-124057.png)

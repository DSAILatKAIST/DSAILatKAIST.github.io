...
permalink: 2024-11-24-Tree-of-Traversals_A_Zero-Shot_Reasoning_Algorithm_for_Augmenting_Black-box_Language_Models_with_Knowledge_Graphs.html
tags: [reviews]
use_math: true
usemathjax: true
---
...
마지막으로 $get$\_$edges(E_{selected}, r)$는 entity set $E_{selected}$와 relation type $r$이 주어졌을 때, $E_{selected}$에서 한 entity를 source entity로 갖고  $r$을 relation type으로 갖는 edge를 추출하여 edge set $T_{added}$ 출력하는 함수이다. 수학적으로 $T_{added}=\{(s,r,o)\in T\mid s\in E_{selected}, r=r, o\in E\}$으로 표현된다. 예를 들어 $E_{selected}$으로 {'Inception', 'Interstellar'}, $r$로 'cast'가 주어졌을 때, ('Inception', 'cast', 'Michael Caine'), ('Interstellar', 'cast', 'Michael Caine') 등의 edge가 출력된다.
...
Algorithm 1은 트리 탐색 알고리즘을 자세히 나타낸 것이다.

1. 먼저 빈 tree $T$와 빈 answer set $Y$를 정의한다.
2. query $q$에 대해서 $initialize(q)$를 수행하면 *default* state를 갖는 노드가 생성되고, 이 노드는 tree $T$의 루트 노드가 된다.
3. 루트 노드에 대해서 k개의 action을 샘플링한다. 구체적으로 *default* state에서는 *Think, Answer, ExpandKG*의 세 가지 action을 취할 수 있으므로 이 중에 샘플링하게 되고, 샘플링은 LLM에 특정 템플릿의 prompt를 입력하여 진행된다. 이때 LLM의 temperature를 조절하여 매 샘플링마다 서로 다른 랜덤한 action을 샘플링할 수 있다. 따라서 각각의 state마다 prompt 템플릿이 존재하고, 아래 그림은 *default* state의 템플릿의  예시를 나타낸 것이다.
![Algorithm](https://i.postimg.cc/fLhHKFLN/prompt-for-default.jpg)

4. 각각의 샘플링 결과에 대해, transition 함수를 적용하여 action을 수행한 결과를 구한다. 예를 들어 *default* state에서 *ExpandKG* action을 수행한 후에는 *selecting-entities* state로 이동하게 된다.
5. k개의 수행 결과는 루트 노드의 자식 노드로 추가되고, value function을 이용하여 각각의 노드의 value를 평가한다. value function은 LLM prompt를 이용하여 평가할 수 있다.
6. 루트 노드를 한 번 방문한 후에는 다시 방문하지 않고, 방문하지 않은 노드 중에서 방문할 노드를 선택한다. 이때 점수가 가장 높은 노드, 점수가 동일할 경우에는 depth가 큰 노드를 선택한다.
7. 선택한 노드에 대해서 마찬가지로 k개의 action을 샘플링하고 자식 노드를 추가한다.
8. 이처럼 노드를 선택하고 k개의 action을 샘플링하는 과정을 반복하여 search tree를 확장하고, 추가된 *Answer* state 노드의 점수가 특정 threshold를 넘으면 해당 노드의 답변이 최종 답변으로 사용된다.
9. 또한 search tree가 무한히 확장하는 것을 방지하기 위해 *max depth*, *max expansions*의 두 가지 stopping criteria를 설정한다. *max depth*에 도달하면 알고리즘이 강제로 *done* state로 이동하여 답변을 생성하고, *max expansions*에 도달하면 모델이 "Cannot find answer"을 답변으로 내놓는다.

Tree-of-Traversals 알고리즘에서 search tree 상에 존재하는 노드의 점수를 산정하기 위해서 LLM을 사용한다. LLM에 특정 템플릿에 맞춰 prompt를 작성하는데, prompt에는 현재의 원래 query, KG subgraph, 이전에 수행한 action 목록과 점수 산정을 위한 instruction이 포함된다. 점수를 산정할 때 일반 노드와 *Answer* state 노드에 서로 다른 prompt를 사용한다. 아래 그림은 일반 노드 및 *Answer* state 노드의 prompt의 예시를 각각 나타낸 것이다.
![Algorithm](https://i.postimg.cc/MH866sXK/general-prompt-for-evaluate.jpg)

![Algorithm](https://i.postimg.cc/Dy8jf2k0/prompt-for-evaluate-on-answer.jpg)
...
Tree-of-Traversals 알고리즘에서 사용할 LLM으로는 Amazon Bedrock에서 API를 통해 이용할 수 있는 Claude-Instant, Llama2 70b, Llama2 13b를 사용한다. 2WikiMultiHop 데이터셋과 QALD-10 데이터셋에서 question answering task를 수행한다.
- Metric
	- Exact Match Included (EM-in)을 사용하는데, 생성한 답변에 정답 답변이 모두 포함되어 있으면 1, 그렇지 않다면 0이다.
	- EM-in은 EM과 다르게 생성한 답변과 정답 답변이 완전히 일치하지 않아도 되는데, LLM이 응답을 생성할 때 다양한 문장 구조와 구문을 사용할 수 있다는 점과 일반적으로 LLM의 답변에 정답 이외에 부가 설명이 추가된다는 점을 고려할 때 더 적합한 metric이다.
- Baseline
	- Tree-of-Traversals 알고리즘의 성능과 비교하기 위한 baseline으로는 Chain-of-Thought (CoT) prompting, ReAct, FLARe와 같이 black-box LLM에서 동작하는 알고리즘을 사용한다.
	- Tree-of-Traversals 알고리즘을 적용한 모델은 두 가지 사용하는데, *branching factor* k=1인 특수한 경우를 Chain-of-Traversals, k=3인 경우를 Tree-of-Traversals라고 칭한다.
- Hyperparameter setting
	- 알고리즘을 수행할 때 search tree가 무한히 확장하는 걸 방지하기 위한 hyperparameter로서 *max depth*는 7, *max expansions*는 20으로 설정한다.
	- 또한 최종 답변을 결정하기 위한 threshold를 0.8로 설정하여 실험을 진행한다.

### 3.1 Question Anwering Task
![Result](https://i.postimg.cc/pdnV8kYn/experiment-result.png)

위 테이블은 2WikiMultiHop 데이터셋과 QALD-10 데이터셋에서 baseline과 Tree-of-Traversals 알고리즘의 성능을 나타낸 것이다.

- 2WikiMultiHop
세 가지 LLM에서 모두 Tree-of-Traversals의 성능이 가장 높게 나타났고, 이 성능은 zero-shot 세팅에서 state-of-the-art에 해당한다. Chain-of-Traversals도 baseline보다 좋은 성능을 보인다는 점에서 ASM을 따라 action을 선택하며 KG subgraph를 추출하는 방식이 효과적이라는 점을 확인할 수 있다. Tree-of-Traversals는 Chain-of-Traversals와 달리 확정적으로 하나의 action을 선택하는 것이 아니라, 가능한 다양한 action 중에서 점수가 높은 action을 선택하여 subgraph를 확장하는 방식으로 최적의 KG subgraph를 추출하기 때문에 두 데이터셋에서 모두 Chain-of-Traversals보다 확연히 좋은 성능을 보였다.
- QALD-10
Llama2-13b를 사용한 경우를 제외하면 Tree-of-Traversals와 Chain-of-Traversals가 baseline보다 좋은 성능을 보였다.
...
만약 value function의 value가 랜덤한 값이었다면, Tree-of-Traversals는 Chain-of-Traversal보다 더 나은 결과를 보이지 못했을 것이다. 이러한 측면에서 value function의 효과를 확인하기 위해 value function에 대한 ablation study가 진행됐다.

왼쪽 그래프는 모델이 0.0 또는 1.0의 value로 평가한 답변에 대한 EM-in 결과를 나타낸 것이다. 이는 0.0의 value를 가진 답변보다 1.0의 value를 가진 답변을 선택할 때 더 정확한 답변이 생성된다는 것을 의미한다.

다음으로 Tree-of-Traversals 알고리즘이 수행하는 backtracking의 효과를 보이기 위한 ablation study의 결과이다. 2WikiMultiHop의 질문 중에서 실제로 Tree-of-Traversals 알고리즘이 답변을 생성하는 과정에서 backtracking이 발생하는 케이스만 선별하여 backtracking을 제한했을 때와 제한하지 않았을 때의 답변을 비교했다. 오른쪽 그래프는 각각의 답변에 대한 EM-in 결과를 나타낸 것이다.

결과적으로 backtracking을 허용했을 때 성능 향상이 있었고, Chain-of-Traversals 알고리즘과 달리 backtracking이 가능한 Tree-of-Traversals 알고리즘의 우수성을 입증한다.
![Result](https://i.postimg.cc/7ZSVs9TM/ablation-studies.jpg)

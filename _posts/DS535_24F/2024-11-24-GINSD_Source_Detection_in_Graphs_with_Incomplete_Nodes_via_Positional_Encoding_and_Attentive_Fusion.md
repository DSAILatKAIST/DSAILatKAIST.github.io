...
permalink: 2024-11-24-GINSD_Source_Detection_in_Graphs_with_Incomplete_Nodes_via_Positional_Encoding_and_Attentive_Fusion.html
tags: [reviews]
use_ math: true
usemathjax: true
---
...
The propagation of the rumor over time is modeled as a time series $\lbrace X(t), t \geq 0 \rbrace$, where $X(t)$ is the state matrix of the graph at time $t$. Each node's state at time $t$ is either:
- $+1$ (rumor received),
...
To validate the necessity of each module in GIN-SD, an ablation study was conducted, showing that removing either the PEM or the class-balancing mechanism resulted in a notable drop in performance. The AFM enhances the model's efficiency in handling heterogenous graphs.
![enter image description here](https://i.postimg.cc/JhK1KCvM/table3.jpg)


## **5. Conclusion**  
GIN-SD innovatively uses the Positional Encoding Module (PEM) to handle incomplete node data by embedding positional information. Compared to previous models tackling this problem, models like LPSI and EPA do not incorporate positional embeddings and solely rely on infection time while models like GCNSI, SIGN, RecGCN use node features and graph structure for representation but do not explicitly calculate positional encodings using Laplacian eigenvectors. Models like IVGD and SL-VAE incorporate diffusion-based learning, which can capture positional relations during propagation however they do not explicitly make use of the structural possition of the nodes as PEM does.

Similarly, using the Attentive Fusion Module (AFM) to prioritize nodes with great transmission capacity through attention mechanisms is a novel concept, with older models using heuristic approaches (LPSI or EPA) or using some kind of propagation data for focusing on critical nodes but lacking the explicit attention approach that is so popular nowadays.


Unlike prior existing models, those modules allow GIN-SD to maintain great results under data loss. The experimental results demonstrated that GIN-SD significantly outperforms state-of-the-art methods across multiple real-world datasets, maintaining high accuracy and F-Score even with incomplete nodes. 

GIN-SD offers a promising solution for handling missing data and incomplete information in source detection task. However, it is computationally intensive for large graphs, particularly because the PEM requires eigenvector calculations. The implementation of the model in this paper also relies on the assumption that the independent cascade model is a good fit for the network that is being analyzed, but this is not a large issue overall as it can be changed accordingly.

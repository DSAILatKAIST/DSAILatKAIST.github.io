...
+ Base-model for this paper: Denoising diffusion probabilistic model (i.e. DDPM)
	...
	+	Structure of DDPM
		+	Forward Process
			...
			$\quad\quad\quad\quad q(x_ {t} \vert x_ {0}) = N \left( x_ {t}; \sqrt{ \bar{ \alpha_ {t}} }x_ {0}, \left(1 - \bar{ \alpha_ {t}} \right) I \right) $
		
			$\quad\quad\quad\quad\quad where \quad \bar{\alpha}_ t = \prod_ {s=1}^{t} \left( 1-\beta_ s \right) $
...
$$
\quad\begin{gathered}
	p_{\theta} \left( \boldsymbol{x}_{0: T} \right) = p \left( \boldsymbol{x}_T \right) \prod_ {t=1}^{T} p \left( \boldsymbol{x} _{t-1} \mid \boldsymbol{x}_ t \right), \\
	\quad p_{\theta} \left( \boldsymbol{x}_ {t-1} \mid \boldsymbol{x}_ t \right) = \mathcal{N} \left( \boldsymbol{x}_ {t-1} ; \boldsymbol{\mu}_{\theta} \left( \boldsymbol{x}_t, t \right), \boldsymbol{\Sigma}_{\theta} \left( \boldsymbol{x}_t, t \right) \right).
\end{gathered}
$$
...
$$
L_{m s e}=\mathbb{E}_{t, \boldsymbol{x}_0, \boldsymbol{\epsilon}}\left[\left(\boldsymbol{\epsilon}-\boldsymbol{\epsilon}_\theta\left(\boldsymbol{x}_t, t\right)\right)^2\right]
$$
...
$$
\begin{gathered}
L_ {v l b}=L_0+L_1+\cdots+L_ {T-1}+L_ T, \\
L_0=-\log p_ \theta \left (\boldsymbol{x}_ 0 \mid \boldsymbol{x}_ 1 \right), \\
L_ {t-1}=D_{K L}\left(q\left(\boldsymbol{x}_{t-1} \mid \boldsymbol{x}_ t, \boldsymbol{x}_ 0 \right) \| p_{\theta} \left( \boldsymbol{x}_ {t-1} \mid \boldsymbol{x}_ t \right) \right), \\
L_ T = D_ {K L} \left( q \left( \boldsymbol{x}_ T \mid \boldsymbol{x}_ 0 \right) \vert p \left( \boldsymbol{x}_ T \right) \right) .
\end{gathered}
$$
...
+ To-Be: Posterior distribution $ q \left(\boldsymbol{x}_ {t-1} \mid \boldsymbol{x}_ t, \boldsymbol{x}_ 0 \right) $와 가정한 distribution $p_\theta\left(\boldsymbol{x}_{t-1} \mid \boldsymbol{x}_t\right)$간의 KL divergence
$$
D_ {K L}\left(q \left(\boldsymbol{x}_ {t-1} \mid \boldsymbol{x}_ t, \boldsymbol{x}_ 0 \right) \vert p_\theta \left( \boldsymbol{x}_ {t-1} \mid \boldsymbol{x}_ t \right) \right)
$$
...
$$
\begin{gathered}
L=\left( f \left( \boldsymbol{x}_ 0 \right) - \boldsymbol{\mu} \right)^{T} \boldsymbol{\Sigma}^{-1}\left( f \left( \boldsymbol{x}_ 0 \right) - \boldsymbol{\mu} \right), \\
\boldsymbol{x}_ {t+1}=\omega \boldsymbol{x}_{t}-s \nabla_ {\boldsymbol{x}_ t} L,
\end{gathered}
$$
	...
	+ Corruption of $\nabla_ {x_t} L$
		+ Noisy Gradient $\nabla_ {x_t} L$ (즉, 무작위의 값이 할당됨) 경우, training iteration을 거듭할수록 image를 다른 방향으로 오염(Corruption)시켜 image에 상당한 noise를 주입하게 될 우려가 있다.
		...
		+ 이러한 방법을 통해, optimization 수행 시 diffusion model의 intermediate step에서 발생한 image들을 diffusion model이 안정적으로 denoise 할 수 있도록 한다. noise $\epsilon_t$의 variance는 상대적으로 작기 때문에, 연구에서는 지정한 몇 개의 $N_ d$번째 optimization step의 image를 denoise하고자 하며, 관련 알고리즘은 아래와 같다.
			...
			| AAAAAAlgorithm 1: Gradient Denoising Reconstruction | 
			| --------------------------------------------------------- | 
			$\quad$**Input**: Image $\boldsymbol{x}_ 0$, Gaussian $(\boldsymbol{\mu},\boldsymbol{\Sigma})$
			$\quad$**Output**: $x_ N$
			    $\quad$ **for** $t=1, \cdots, N$ **do**
			    $\quad$ $\quad$ $\boldsymbol{f}_ {t} = F \left( \boldsymbol{x}_ t \right)$
			    $\quad$ $\quad$ $\boldsymbol{g} = \nabla_ {\boldsymbol{x}_ {t}} \left( \boldsymbol{f}_ {t} - \boldsymbol{\mu} \right)^{T} \boldsymbol{\Sigma}^{-1} \left( \boldsymbol{f}_ {t}-\boldsymbol{\mu} \right)$
			    $\quad$ $\quad$ $\boldsymbol{x}_ t = \sqrt{1-\hat{\beta}_ t} \boldsymbol{x}_ {t-1} + \sqrt{\hat{\beta}_ t} \boldsymbol{g} ;$
			    $\quad$ $\quad$ **if** $t \% N_ {d}=0$ **then**
			    $\quad$ $\quad$ $\quad$$x_ {t} \sim \mathcal{N} \left( \boldsymbol{\mu}_ {\boldsymbol{\theta}} \left( \boldsymbol{x}_ {t} \right), \boldsymbol{\sigma}_ {\theta} \left(\boldsymbol{x}_ {t} \right) \right)$
			    $\quad$ $\quad$**end**
			    $\quad$**end**
...
2. Diffusion model의 각 step 마다의 KL divergence를 활용함으로써 더 우수한 localization이 가능한 boundary-aware result를 도출하였다.

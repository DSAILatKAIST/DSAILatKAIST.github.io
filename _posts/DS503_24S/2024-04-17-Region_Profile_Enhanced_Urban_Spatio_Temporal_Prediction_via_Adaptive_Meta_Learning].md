...
- 글로벌 업데이트 (Global Update): 공유되는 일반화 가능한 예측 모델 $\theta^*$와 적은 수의 샘플 태스크에 적합한 효과적인 편향 생성기 $\eta^*$를 얻는 것을 목표로 한다. 이는 프로파일과 ST 동태를 연결한다. 로컬 업데이트 후, 한 단계의 그래디언트 감소를 통해 두 파라미터를 쿼리 세트 $D_{ri}^{qr}$의 손실 합계에 기반하여 업데이트한다.
